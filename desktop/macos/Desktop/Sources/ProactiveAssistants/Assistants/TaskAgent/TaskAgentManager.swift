...
class TaskAgentManager: ObservableObject {
      ...
      while !Task.isCancelled {
        ...
        if output == lastOutput {
          ...
          if unchangedCount >= self.idleThreshold {
            ...
            if let s = self.activeSessions[taskId] { self.persistSession(s) }
            logMessage(
              "TaskAgentManager: Session idle for task \(taskId) (output unchanged for \(unchangedCount) polls, \(editedFiles.count) files edited), stopping poll"
            )
            break
          }
        } else {
  ...
  nonisolated private static func readTmuxOutput(sessionName: String) -> String {
    ...
    process.executableURL = URL(fileURLWithPath: "/bin/zsh")
    process.arguments = [
      "-c", "source ~/.zprofile 2>/dev/null; tmux capture-pane -t '\(sessionName)' -p -S -500 2>/dev/null",
    ]
