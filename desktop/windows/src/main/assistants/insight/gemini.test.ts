...
describe('runTwoPhasePipeline', () => {
  it('routes self-hosted insight through the operator generic capability only', async () => {
    selfHostedProfile()
    h.fetch.mockResolvedValue(capabilityResult('no_advice', {}))

    await expect(runTwoPhasePipeline(deps({ session: selfHostedSession() }))).resolves.toEqual({
      insight: null,
      sqlCount: 0
    })
