...
import { isBetterAuthEnabled, signInWithEmail, signUpWithEmail } from '@/lib/better-auth';

export function LoginClient() {
  const { user, loading, socialProviders, signInWithGoogle, signInWithApple } = useAuth();
  ...
  const canUseGoogle = !isBetterAuthEnabled || socialProviders.includes('google');
  const canUseApple = !isBetterAuthEnabled || socialProviders.includes('apple');
            ...
            >
              {isBetterAuthEnabled && (
                <form onSubmit={handleEmailSubmit} className="space-y-3">
                  {isSignUp && (
                    <label className="block text-left text-sm text-text-secondary">
                      Name
                      <input
                        value={name}
                        ...
                        required
                        autoComplete="name"
                        className="mt-1 w-full rounded-xl border border-white/15 bg-black/50 px-4 py-3 text-white outline-none focus:border-white/40"
                      />
                    </label>
                  )}
                  <label className="block text-left text-sm text-text-secondary">
                  ...
                  <button
                    ...
                    className="w-full rounded-xl bg-white px-6 py-4 font-medium text-black transition-all hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    {isEmailSubmitting
                      ? 'Connecting...'
                      : isSignUp
                  ...
                  <button
                    ...
                    className="w-full py-2 text-sm text-text-secondary underline decoration-text-secondary/40 hover:text-text-primary"
                  >
                    {isSignUp
                      ? 'Already have an account? Sign in'
                      : 'New to Omi? Create an account'}
                  </button>
                </form>
              )}

              {(canUseApple || canUseGoogle) && (
                <>
                  {/* Apple Sign In */}
                  {canUseApple && (
                    <button
