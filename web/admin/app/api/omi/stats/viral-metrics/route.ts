...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const growthAccounting = allWeeks.map((week) => {
      ...
      const prevActive =
        prevWeekIdx >= 0 ? (weeklyActive[allWeeks[prevWeekIdx]] ?? 0) : 0;
    ...
    const totalChurnedGA = recentGA.reduce(
      (s, w) => s + Math.abs(w.churned),
      0,
    );
    const quickRatio =
      totalChurnedGA > 0
        ? Math.round(
            ((totalNewGA + totalResurrectedGA) / totalChurnedGA) * 100,
          ) / 100
        : null;
