...
def _embed_with_projection_descriptor(request: EmbeddingCapabilityRequest) -> tuple[list[list[float]], Any]:
    ...
    descriptor = describe_active_projection(cast(Any, embeddings), dimension=dimension, capability='embedding')
...
def _invoke_tool_completion(request: ToolCompletionRequest, route: ModelCapabilityRoute) -> Any:
    model = (
        get_llm('desktop_proactive_reasoning', request_timeout=30)
        if should_route_features_through_gateway()
