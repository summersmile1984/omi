...
from .llm_http import DeadlineTransport, Frames, arequest_json, request_json, validate_response
...
class LocalChatModel(BaseChatModel):
    ...
    def payload(self, messages, stop, stream, kwargs):
        ...
        allowed = {'tools', 'tool_choice', 'max_completion_tokens', 'max_tokens', 'format'}
    ...
    async def _astream(self, messages, stop=None, run_manager=None, **kwargs):
        ...
        authority = self.authority
        complete, emitted, total, call_index = False, False, 0, 0
        try:
            async with asyncio.timeout(authority.timeout):
                async with httpx.AsyncClient(
                    transport=authority.transport,
                    ...
                    async with client.stream('POST', authority.endpoint + '/api/chat', json=payload) as response:
                        ...
                        async for data in events():
                            ...
                            text, calls, usage = self.decode(data)
                            emitted = emitted or bool(text.strip() or calls)
                            ...
                            yield ChatGenerationChunk(
                                message=chunk, generation_info={'model_name': authority.contract.model}
                            )
            if not complete or not emitted:
