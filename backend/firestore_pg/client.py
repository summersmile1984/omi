...
class DocumentReference:
    ...
    def get(
        self, field_paths: Optional[List[str]] = None, transaction: Optional["Transaction"] = None
    ) -> DocumentSnapshot:
        if transaction is not None:
    ...
    def update(self, field_updates: Mapping[str, Any]) -> "DocumentReference":
        ...
        def _do(conn: Any) -> None:
            row = conn.execute(text(get_sql(self._table)), {"uid": self._uid or "", "doc_id": self._id}).fetchone()
            if row is None:
    ...
    def delete(self) -> "DocumentReference":
        _run_with_conn(
            lambda conn: conn.execute(text(delete_sql(self._table)), {"uid": self._uid or "", "doc_id": self._id}),
            table=self._table,
        )
        return self
...
class Transaction:
    ...
    def delete(self, ref: DocumentReference) -> None:
        _run_with_conn(
            lambda conn: conn.execute(text(delete_sql(ref._table)), {"uid": ref._uid or "", "doc_id": ref._id}),
            table=ref._table,
        )
