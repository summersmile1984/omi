...
**Firestore** (primary store; PostgreSQL-compatible local shim): use `get_firestore_client()` from `database._client` at call time, and add optional keyword-only `firestore_client` parameters on converted database helpers so tests can inject fake clients. `FIRESTORE_PG_DSN` selects `firestore_pg` for the whole runtime; never split a process between PostgreSQL and Firestore. Neutral/self-hosted launches require explicit `FIRESTORE_PG_DSN` or `FIRESTORE_EMULATOR_HOST`; no ADC/managed fallback. `db` is legacy lazy compatibility only; do not use it in new code. Never construct Firestore clients at import time. Collection-group queries need indexes; segments are encrypted at rest — direct Firestore reads return opaque blobs. Feature gating via user fields: e.g., translation requires `users/{uid}.language` non-empty — silently disabled if missing.
