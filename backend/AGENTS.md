...
- **cloud-neutral local runtime** — PostgreSQL, MinIO, leased Redis workers, Better Auth, and SenseVoice replace the managed data/identity/speech plane. Mutable provider configuration is read at call boundaries; the exact production env, migration, and operations contract is in `../deploy/self-host/README.md`.
- **model-neutral runtime** — direct/gateway routes, typed desktop model endpoints, versioned projections, and the allowlisted `omi.realtime.v1` relay fail closed; self-host disables app-icon and legacy Files/Assistants. Use `scripts/vector_projection_migration.py` only with authoritative exports; the cutover procedure is in `../deploy/self-host/README.md`.
...
- **diarizer** (`diarizer/main.py`) — GPU. Speaker embeddings at `/v2/embedding`. Called by backend and pusher when `SPEAKER_EMBEDDING_PROVIDER=http` using `SPEAKER_EMBEDDING_API_URL`; self-host may explicitly select `disabled` while retaining transcription without speaker identity.
