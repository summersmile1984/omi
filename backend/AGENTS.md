...
Key env vars: `OPENAI_API_KEY` (LLM calls), `HOSTED_PARAKEET_API_URL` / `MODULATE_API_KEY` (default STT), `DEEPGRAM_API_KEY` with its self-hosted endpoint, `GEMINI_API_KEY` / `ANTHROPIC_API_KEY` (desktop chat), `ENCRYPTION_SECRET` (tests), and `REDIS_DB_HOST` (fail-open rate limiting). `OMI_SHARE_BASE_URL` is mandatory for neutral/self-hosted deployments and must be an operator-owned HTTPS origin; managed deployments retain the `https://h.omi.me` default. `PUSH_PROVIDER` defaults to Firebase only for managed profiles; a neutral/self-hosted profile with the variable omitted resolves to `disabled` before Firebase initialization. Optional `PUSH_PROVIDER=webhook` requires an HTTPS `PUSH_WEBHOOK_URL` plus a regular mode-`0600` `PUSH_WEBHOOK_SECRET_FILE`; it uses the signed `omi.push.webhook.v1` / receipt contract and never falls through to Firebase. `SERVICE_ACCOUNT_JSON` / `GOOGLE_APPLICATION_CREDENTIALS` are default Firebase Admin credentials; never commit them. Credential files must be regular, non-symlink, mode `0600`; inline JSON is owner-only. `SERVICE_ACCOUNT_JSON` pins Firestore/GCS to its project; desktop GCE/agentVm may retain ADC, while quota/llm_usage use the customer SA. Self-hosted Compose sets `AGENT_VM_PROVIDER=disabled`; deletion never discovers ADC there and fails closed on legacy VM state.
