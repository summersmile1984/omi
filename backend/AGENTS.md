...
Key env vars: `OPENAI_API_KEY`, `HOSTED_PARAKEET_API_URL` / `MODULATE_API_KEY`, `DEEPGRAM_API_KEY`, `GEMINI_API_KEY` / `ANTHROPIC_API_KEY`, `ENCRYPTION_SECRET`, and `REDIS_DB_HOST`. `OMI_SHARE_BASE_URL` is mandatory for neutral/self-hosted deployments and must be an operator-owned HTTPS origin; managed deployments retain `https://h.omi.me`. `PUSH_PROVIDER` defaults to Firebase only for managed profiles; neutral/self-hosted omission resolves to `disabled` before Firebase initialization. `PUSH_PROVIDER=webhook` requires HTTPS `PUSH_WEBHOOK_URL` plus a regular mode-`0600` `PUSH_WEBHOOK_SECRET_FILE`; it uses the signed `omi.push.webhook.v1` / receipt contract and never falls through to Firebase. `SERVICE_ACCOUNT_JSON` / `GOOGLE_APPLICATION_CREDENTIALS` are default Firebase Admin credentials; never commit them. Credential files must be regular, non-symlink, mode `0600`; inline JSON is owner-only. `SERVICE_ACCOUNT_JSON` pins Firestore/GCS to its project; desktop GCE/agentVm may retain ADC, while quota/llm_usage use the customer SA. Self-hosted Compose sets `AGENT_VM_PROVIDER=disabled`; deletion never discovers ADC there and fails closed on legacy VM state.
...
- **hosted MCP OAuth** (`routers/mcp_sse.py`) — Provider-neutral OAuth for `/v1/mcp/sse`. Configure public or confidential clients with `MCP_OAUTH_CLIENTS_JSON`; allowlist the exact connector callback URI from the provider. The temporary `MCP_OAUTH_CHATGPT_*` envs still define the legacy confidential ChatGPT test client, and `MCP_OAUTH_PUBLIC_*` can expose a no-secret PKCE public client. Managed deployments retain the historical `https://api.omi.me/v1/mcp/sse` default; neutral/self-hosted deployments must set an explicit operator-owned `MCP_RESOURCE_URL` or `PUBLIC_MCP_URL` (the latter derives `/v1/mcp/sse`). Missing/invalid authority is a typed unavailable capability and OAuth discovery/authorize/token return 503; it never falls back to an Omi URL. Also set `MCP_AUTHORIZATION_SERVER_URL` and token TTL env vars.
