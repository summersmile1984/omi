...
- **self-host runtime** — PostgreSQL, MinIO, Redis, Better Auth and local model routes replace managed services; contract: `../deploy/self-host/README.md`.
...
- **operator-owned mlx-audio MOSS** (`utils/mlx_moss_diarize/`) — Batch STT with explicit endpoint/model and no hosted fallback; long audio requires mounted `sherpa_onnx` embeddings. See the self-host runbook.
