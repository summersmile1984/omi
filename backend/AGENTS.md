...
- **self-host runtime** — PostgreSQL, MinIO, leased Redis workers, Better Auth, SenseVoice, provider-neutral model routes, versioned projections, and `omi.realtime.v1` replace managed dependencies. Providers are call-boundary selected and fail closed; environment, migration, and operations contracts live in `../deploy/self-host/README.md`.
