...
Key env vars: `OPENAI_API_KEY` (LLM calls), `HOSTED_PARAKEET_API_URL` / `MODULATE_API_KEY` (default STT), `DEEPGRAM_API_KEY` with its self-hosted endpoint, `GEMINI_API_KEY` / `ANTHROPIC_API_KEY` (desktop chat), `ENCRYPTION_SECRET` (tests), and `REDIS_DB_HOST` (fail-open rate limiting). `PUSH_PROVIDER` defaults to Firebase only for managed profiles; a neutral/self-hosted profile with the variable omitted resolves to `disabled` before Firebase initialization. `SERVICE_ACCOUNT_JSON` / `GOOGLE_APPLICATION_CREDENTIALS` are default Firebase Admin credentials; never commit them. File-based credential paths, including `FIREBASE_AUTH_CREDENTIALS_PATH`, must be regular non-symlink files with mode `0600` or stricter; inline JSON is materialized with owner-only permissions. When `SERVICE_ACCOUNT_JSON` is set, Firestore (and GCS) pin that customer-data SA and its `project_id` so GKE Workload Identity / a compute-project `GOOGLE_CLOUD_PROJECT` cannot silently win user-doc reads. Dev desktop-backend keeps ADC on the compute project for GCE/`agentVm`; quota and `llm_usage` use `get_customer_firestore_client()` from the Auth SA file (`FIREBASE_AUTH_CREDENTIALS_PATH`) so Beta sees production `based-hardware` entitlements without writing `agentVm` there. Self-hosted Compose sets `AGENT_VM_PROVIDER=disabled`; account deletion never discovers GCP ADC in that mode and fails closed if legacy VM state still needs reconciliation.
