...
- **cloud-neutral local runtime** — `FIRESTORE_PG_DSN` routes both customer and compute data through `firestore_pg`; `STORAGE_BACKEND=minio` selects the GCS-compatible MinIO adapter (`MINIO_PUBLIC_ENDPOINT` owns returned/signed URLs); `QUEUE_BACKEND=redis` selects leased Redis workers authenticated by the queue-specific `QUEUE_REDIS_{SYNC|AUDIO_MERGE|ACCOUNT_DELETION|FINALIZATION}_WORKER_SECRET`; `AUTH_PROVIDER=better_auth` verifies asymmetric JWTs from `AUTH_JWKS_URL`. The self-host profile sets both `STT_SERVICE_MODELS=sensevoice` and `STT_PRERECORDED_MODEL=sensevoice`; live decoding uses bounded local PCM windows in `sync_executor`, admission requires the locked `sherpa-onnx` runtime plus both model files, and `STT_ROUTE_FALLBACK_TO_DEFAULT=false` prevents a missing model from selecting a managed default. Mutable provider configuration is read at call boundaries.
- **model-neutral runtime** — `OMI_LLM_DEFAULT_{PROVIDER,MODEL,FALLBACKS}` defines the deployment route, while `OMI_LLM_ROUTE_<FEATURE>_{PROVIDER,MODEL,FALLBACKS}` overrides one workload; OpenAI-compatible legs never borrow `OPENAI_API_KEY`, and `generic` additionally requires its own key plus `GENERIC_OPENAI_BASE_URL` and never defaults to an official endpoint. `EMBEDDING_PROVIDER` and `VECTOR_STORE_PROVIDER` select the retrieval adapters; Qdrant requires `QDRANT_URL` or `QDRANT_PATH`. Vector writes stamp provider/model/dimension/schema/namespace identity. Operators migrate only from immutable authoritative JSONL exports: set `VECTOR_PROJECTION_MODE=dual_write` with distinct active/target versions retained in `VECTOR_PROJECTION_DELETE_VERSIONS`, declare every global switch namespace in `VECTOR_PROJECTION_REQUIRED_NAMESPACES`, then run `python scripts/vector_projection_migration.py {validate,backfill,verify,switch,rollback}`. Source-bound receipts plus online identity/vector/count verification must pass for the complete switch plan before an overlay is emitted.
