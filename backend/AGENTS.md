...
Key env vars: `OPENAI_API_KEY`, `OPENROUTER_API_KEY`, `GENERIC_OPENAI_BASE_URL`/`GENERIC_OPENAI_API_KEY`, `EMBEDDING_PROVIDER`/`EMBEDDING_MODEL`, `REALTIME_PROVIDER`/`REALTIME_MODEL` or `REALTIME_RELAY_*`, STT settings, `GEMINI_API_KEY`/`ANTHROPIC_API_KEY`, `ENCRYPTION_SECRET`, and `REDIS_DB_HOST`. Neutral deployments set `OMI_DEPLOYMENT_PROFILE=self_hosted` plus explicit routes; managed routes are rejected and capability endpoints return typed unavailable until configured. `SERVICE_ACCOUNT_JSON`/`GOOGLE_APPLICATION_CREDENTIALS` are Firebase Admin credentials; never commit them. Dev deploys use `FIREBASE_AUTH_CREDENTIALS_PATH` only for token verification.
...
- **hosted MCP OAuth** (`routers/mcp_sse.py`) — `/v1/mcp/sse` uses exact client callbacks. Managed keeps `api.omi.me`; neutral/self-hosted requires operator `MCP_RESOURCE_URL` or `PUBLIC_MCP_URL`, else discovery/authorize/token return typed 503.
- **desktop updates** (`routers/updates.py`, `routers/desktop_tts_updates.py`) — neutral/self-hosted never consult managed release authorities; legacy routes return typed unavailable. Operator download policy requires an explicit HTTPS URL.
...
- **diarizer** (`diarizer/main.py`) — GPU `/v2/embedding`; self-host selects `sherpa_onnx`. Both require explicit endpoints/models and never download at runtime.
...
- **operator-owned mlx-audio MOSS** (`utils/mlx_moss_diarize/`) — Batch STT with explicit endpoint/model, no hosted fallback; long audio requires mounted `sherpa_onnx` (see self-host runbook).
...
**Firestore** (primary store; PostgreSQL-compatible local shim): use `get_firestore_client()` at call time and allow fake-client injection in converted helpers. `FIRESTORE_PG_DSN` selects `firestore_pg` for the whole runtime; never split a process between PostgreSQL and Firestore. Neutral/self-hosted launches require explicit `FIRESTORE_PG_DSN` or `FIRESTORE_EMULATOR_HOST`; no ADC/managed fallback. `db` is legacy lazy compatibility only. Never construct clients at import time. Collection-group queries need indexes; segments are encrypted at rest.

**Object storage**: managed may use GCS; neutral/self-hosted requires `STORAGE_BACKEND=minio` and never falls back to GCS.
...
`AUTH_PROVIDER=better_auth` is self-hosted; neutral/self-hosted must set it explicitly (omission fails closed). JWT verification accepts asymmetric keys from `AUTH_JWKS_URL`; `/auth-issue` stays disabled unless protected by `AUTH_DEV_ISSUER_SECRET`.
