...
Self-host projection migrations are source-first and read-only at the authority
boundary. `scripts/export_authoritative_vectors.py` emits one strict JSONL
inventory per logical namespace from Firestore or the PostgreSQL facade; it
never reads a vector index and requires explicit `--uid` or `--all-users`.
`scripts/vector_projection_migration.py` is the only command allowed to embed
that verified inventory and create a projection receipt. Storage and identity
imports follow the same dry-run/apply/verify rule in `deploy/self-host/README.md`.
...
- **self-host runtime** — PostgreSQL, MinIO, Redis workers, Better Auth, local speech/model routes, versioned projections, and `omi.realtime.v1` replace managed dependencies. The fail-closed deployment contract lives in `../deploy/self-host/README.md`.
...
- **operator-owned mlx-audio MOSS diarization** (`utils/mlx_moss_diarize/`) — Batch-only STT with explicit endpoint/model and no hosted fallback. Long diarized audio requires mounted `sherpa_onnx` embeddings for cross-chunk speaker reconciliation; see the self-host runbook.
