...
Key env vars: `OPENAI_API_KEY`, `OPENROUTER_API_KEY`, `GENERIC_OPENAI_BASE_URL` + `GENERIC_OPENAI_API_KEY` (operator route), `EMBEDDING_PROVIDER`/`EMBEDDING_MODEL`, `EMBEDDING_CAPABILITY_TRANSPORT=direct`, `REALTIME_PROVIDER`/`REALTIME_MODEL` or allowlisted `REALTIME_RELAY_*`, hosted/self-hosted STT settings, `GEMINI_API_KEY`/`ANTHROPIC_API_KEY`, `ENCRYPTION_SECRET`, and `REDIS_DB_HOST`. Neutral deployments set `OMI_DEPLOYMENT_PROFILE=self_hosted` plus explicit default/per-feature routes; managed routes are rejected and capability endpoints return typed unavailable until configured. `SERVICE_ACCOUNT_JSON` / `GOOGLE_APPLICATION_CREDENTIALS` are Firebase Admin credentials; never commit them. Dev desktop deploys use `FIREBASE_AUTH_CREDENTIALS_PATH` only for token verification.
...
- **hosted MCP OAuth** (`routers/mcp_sse.py`) — Provider-neutral `/v1/mcp/sse` OAuth. Configure `MCP_OAUTH_CLIENTS_JSON` and exact connector callbacks. Managed keeps the historical `https://api.omi.me/v1/mcp/sse`; neutral/self-hosted requires an operator `MCP_RESOURCE_URL` or `PUBLIC_MCP_URL` (derives `/v1/mcp/sse`). Missing/invalid authority is typed unavailable; discovery/authorize/token return 503, never an Omi fallback. Also set `MCP_AUTHORIZATION_SERVER_URL` and token TTL env vars.
- **desktop updates** (`routers/updates.py`, `routers/desktop_tts_updates.py`) — neutral/self-hosted requests never consult managed Firestore/GitHub release authorities or return their URLs; legacy appcast/download routes return typed `operator_update_authority_not_configured`. Operator download policy is separate and must carry an explicit operator-owned HTTPS URL.
...
- **diarizer** (`diarizer/main.py`) — GPU speaker embeddings at `/v2/embedding`. Backend and pusher select `http`; self-host selects `sherpa_onnx`. Both require explicit endpoints/models and never download at runtime.
...
- **operator-owned mlx-audio MOSS** (`utils/mlx_moss_diarize/`) — Batch STT with explicit endpoint/model, no hosted fallback; long audio requires mounted `sherpa_onnx`. See the self-host runbook.
