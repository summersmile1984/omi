...
- **self-host runtime** — PostgreSQL, MinIO, leased Redis workers, Better Auth, SenseVoice, provider-neutral model routes (including private local attachment extraction), versioned projections, and `omi.realtime.v1` replace managed dependencies. Providers are call-boundary selected and fail closed; environment, migration, and operations contracts live in `../deploy/self-host/README.md`.
...
- **diarizer** (`diarizer/main.py`) — GPU. Speaker embeddings at `/v2/embedding`. Called by backend and pusher when `SPEAKER_EMBEDDING_PROVIDER=http` using `SPEAKER_EMBEDDING_API_URL`; CPU self-host selects `sherpa_onnx` with an explicit mounted `SPEAKER_EMBEDDING_MODEL`, and neither mode downloads a model at runtime.
...
- **operator-owned mlx-audio MOSS diarization** (`utils/mlx_moss_diarize/`) — Batch-only multipart STT at an explicit `MLX_MOSS_DIARIZE_ENDPOINT` and `MLX_MOSS_DIARIZE_MODEL`. It is a separate authority from hosted `mosi.cn` MOSS, has no default or hosted fallback, and caps requests at four minutes before chunking only when mounted `sherpa_onnx` embeddings can reconcile request-local speaker IDs globally; otherwise long diarized audio fails closed.
