...
- **model-neutral runtime** — global, per-feature, and fallback LLM routes share one direct/gateway contract; embedding and vector adapters stamp versioned projection identity. Use `scripts/vector_projection_migration.py` only with authoritative exports; the full fail-closed cutover procedure is in `../deploy/self-host/README.md`.
