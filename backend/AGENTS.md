...
Key env vars: `OPENAI_API_KEY` (LLM calls), `OPENROUTER_API_KEY` (managed OpenRouter), `GENERIC_OPENAI_BASE_URL` + `GENERIC_OPENAI_API_KEY` (operator-owned generic route), `EMBEDDING_PROVIDER` + `EMBEDDING_MODEL` (provider-neutral embeddings; self-hosted deployments must set these explicitly), `EMBEDDING_CAPABILITY_TRANSPORT=direct`, `REALTIME_PROVIDER`/`REALTIME_MODEL` or the allowlisted `REALTIME_RELAY_*` contract (provider-neutral realtime), `HOSTED_PARAKEET_API_URL` / `MODULATE_API_KEY` (default STT), `DEEPGRAM_API_KEY` with its self-hosted endpoint, `GEMINI_API_KEY` / `ANTHROPIC_API_KEY` (desktop chat), `ENCRYPTION_SECRET` (tests), and `REDIS_DB_HOST` (fail-open rate limiting). Neutral deployments must set `OMI_DEPLOYMENT_PROFILE=self_hosted` and explicit `OMI_LLM_DEFAULT_PROVIDER`/`OMI_LLM_DEFAULT_MODEL` (or per-feature route variables); checked-in managed routes are rejected and capability endpoints return typed unavailable responses until an operator route is configured. `SERVICE_ACCOUNT_JSON` / `GOOGLE_APPLICATION_CREDENTIALS` are default Firebase Admin credentials; never commit them. Dev desktop deploys use `FIREBASE_AUTH_CREDENTIALS_PATH` only for Firebase token verification, leaving Google clients on dev Cloud Run ADC.
