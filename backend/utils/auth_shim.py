...
def _fetch_jwks() -> Dict[str, Any]:
    ...
    with _jwks_lock:
        ...
        try:
            ...
            # A bounded stale window absorbs a brief control-plane outage, but
            # never lets removed keys remain trusted indefinitely.
            if _jwks is not None and _jwks_source_url == url and (now - _jwks_fetched_at) <= _jwks_max_stale():
