...
DIRECT_PROVIDER_ALLOWLIST = {
    ...
    DirectUse('utils/llm/embedding_providers.py', 'OpenAIEmbeddings'),
...
def test_generated_gateway_lanes_apply_only_declared_gateway_route_overrides():
    ...
    for feature in get_all_configured_features():
        if feature == 'public_shared_conversation_chat':
            # This gateway-only endpoint keeps its dedicated non-streaming,
            # no-fallback route artifact instead of a generated artifact.
            continue
        ...
        assert [(fallback.provider, fallback.model) for fallback in route.fallbacks] == [
            (fallback.provider, fallback.model) for fallback in resolve_feature_route(feature).fallbacks
        ]
...
def test_anthropic_generated_lanes_do_not_advertise_streaming_without_adapter_support():
    ...
    assert chat_agent.surface == Surface.ANTHROPIC_MESSAGES
