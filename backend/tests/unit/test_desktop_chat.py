...
async def test_chat_completions_routes_public_web_search_to_managed_perplexity_lane(monkeypatch):
    ...
    # The neutral direct-search gate would divert an omi_web_search turn to the
    # OpenAI-compatible/searxng path; these tests pin the managed gateway lane
    # selection (web-search → Perplexity), so keep it off.
    monkeypatch.setattr(desktop_chat, '_direct_web_search_requested', lambda _body: False)
    monkeypatch.setattr(
        desktop_chat,
...
async def test_chat_completions_routes_pi_public_web_policy_to_managed_perplexity_lane(monkeypatch):
    ...
    monkeypatch.setattr(desktop_chat, '_record_usage', lambda *_args, **_kwargs: _done())
    monkeypatch.setattr(desktop_chat, 'get_llm_gateway_base_url', lambda: 'http://gateway.test')
    # See test_chat_completions_routes_public_web_search_to_managed_perplexity_lane.
    monkeypatch.setattr(desktop_chat, '_direct_web_search_requested', lambda _body: False)
    monkeypatch.setattr(
        desktop_chat,
        'get_direct_anthropic_client',
