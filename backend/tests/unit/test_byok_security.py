...
class TestRequestHasLLMByokKey:
    def test_accepts_openrouter_and_gemini(self, monkeypatch):
        ...
        assert subscription.request_has_llm_byok_key() is True
        # Deepgram is a transcription capability, never an LLM capability.
        _enrolled({'deepgram': 'dg-key'})
        assert subscription.request_has_llm_byok_key() is False

    def test_unenrolled_llm_key_does_not_count(self, monkeypatch):
        """d0e3a4eb3a: an LLM key must be enrolled, not merely present.

        A raw validated OpenRouter header with no matching entry in the user's
        enrolled fingerprints must NOT unlock the chat-quota bypass.
        """
        from utils import subscription

        monkeypatch.setattr(subscription, 'has_validated_byok_keys', lambda: True)
        monkeypatch.setattr(subscription, 'get_byok_uid', lambda: 'uid-1')
        monkeypatch.setattr(subscription, 'get_cached_byok_state', lambda _uid: {'fingerprints': {}})
        monkeypatch.setattr(
...
class TestCacheRouting:
    ...
    def test_non_gemini_openrouter_routes_through_openrouter(self, monkeypatch):
        """Non-Gemini OpenRouter BYOK creates a client on the OpenRouter endpoint.

        Upstream merge (clients.py openrouter branch) supports every OpenRouter
        model through openrouter.ai/v1 with the X-Title header; only Gemini
        models gain the google/ prefix. The old fails-closed expectation for a
        non-Gemini OpenRouter model is now the positive path — the fail-closed
        tail raise is pinned separately for providers outside the BYOK
        transport set.
        """
        clients, stub_client, _constructor = self._stub_openai_constructor(monkeypatch)

        client = clients._create_byok_client('anthropic/claude-3.5-sonnet', 'openrouter', 'sk-or-key')
        assert isinstance(client, stub_client)
        assert client.openai_api_base == 'https://openrouter.ai/api/v1'
        assert client.model_name == 'anthropic/claude-3.5-sonnet'
        _constructor.assert_called_once()
        assert _constructor.call_args.kwargs['api_key'] == 'sk-or-key'
        assert _constructor.call_args.kwargs['default_headers'] == {'X-Title': 'Omi Chat'}

    def test_unsupported_byok_transport_fails_closed(self):
        """A BYOK transport outside the supported set must fail closed.

        The merged clients.py tail raise guarantees an unsupported BYOK
        transport never silently constructs any client — platform or user.
        """
        ...
        with pytest.raises(ModelProviderConfigurationError) as raised:
            _create_byok_client('gpt-4o-mini', 'unsupported-provider', 'sk-key')

        assert raised.value.provider == 'unsupported-provider'
    ...
    def test_get_llm_unsupported_byok_never_constructs_platform_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            clients,
            '_get_model_config',
            lambda _feature: ('gpt-4o-mini', 'unsupported-provider'),
        )
        monkeypatch.setattr(
            clients, 'get_byok_key', lambda provider: 'user-key' if provider == 'unsupported-provider' else None
        )
...
class TestBYOKFingerprintValidation:
    ...
    def test_missing_enrolled_provider_header_raises_403(self, mock_get_state):
        ...
        Upstream 1da8880175 validates every enrolled provider, not just the sent
        headers: a partial-header BYOK attempt fails closed with a 403.
        """
        from fastapi import HTTPException
        from utils.byok import _byok_ctx, validate_byok_request
