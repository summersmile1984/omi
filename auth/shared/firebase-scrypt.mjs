...
function parseFirebaseScryptConfig(raw, limits) {
  ...
  const rounds = positiveInteger(raw.rounds, "rounds", limits.maxRounds);
  const memCost = positiveInteger(
    raw.mem_cost ?? raw.memCost,
    "mem_cost",
    MAX_MEM_COST,
  );
  if (128 * 2 ** memCost * rounds > limits.maxMemoryBytes) {
    throw new FirebasePasswordMigrationConfigurationError(
...
function firebaseScryptConfigFromEnv(env, limits) {
...
async function verifyMigratedFirebasePassword({ hash, password }, env, limits) {
  const envelope = parseFirebasePasswordHash(hash);
  const config = firebaseScryptConfigFromEnv(env, limits);
  if (!config) {
    throw new FirebasePasswordMigrationConfigurationError(
  ...
  const candidate = await hashFirebasePassword(password, envelope.salt, config);
...
function firebaseScryptPolicy(limits) {
  return Object.freeze({
    parseConfig: (raw) => parseFirebaseScryptConfig(raw, limits),
    fromEnv: (env) => firebaseScryptConfigFromEnv(env, limits),
    verify: (credentials, env) =>
      verifyMigratedFirebasePassword(credentials, env, limits),
  });
}

// Preserve each existing runtime's admitted principal parameters. The Workers
// budget is also used by its importer, before any credential is written.
