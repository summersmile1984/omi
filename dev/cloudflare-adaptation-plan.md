...
- `manifests/routes.yaml` 与 `manifests/resources.yaml`：Cloudflare route/resource inventory；`manifests/backend-routes.json` 由 hermetic FastAPI import 生成并逐条记录 577 条唯一后端路由，其中 75 条显式保留 legacy owner，其余匹配 staging Worker owner 或外部/blocked seam。新注册路由在生成时保持 `unclassified`，OpenAPI CI 与 manifest 契约会拒绝未审查状态。`redis-primitives.yaml` 将 `backend/database/redis_db.py` 的公开 helper（另显式豁免装饰器）归入 34 个 key family，并覆盖绕过 helper 或直接依赖 Redis package 的生产/工具调用点；`vector-namespaces.yaml` 覆盖 7 个现有 Pinecone namespace，其中 memory、action-item、conversation summary、transcript chunk 与 X posts 五个多语言 BGE-M3/1024 维 projection 已由 staging Vectorize 接管；`r2-namespaces.yaml` 覆盖 9 个 `BUCKET_*` 对象 namespace。`npm test` 前置校验会检查字段、命名空间、重复项、禁止 broad `/v1/*` ownership、Edge 路由表示、完整 backend route parity、源码清单完整性、Vectorize 维度/re-embedding 契约、R2 环境隔离，以及 Cloudflare Worker 禁止连接 Redis。Edge 只把显式迁移的 route 送入 partial Worker，未迁移的认证 route 在配置 `LEGACY_BACKEND_URL` 时回旧后端。

已执行并通过：

```text
npm run typecheck                         # pass
npm test                                  # 15 files / 131 tests pass
uvx uv==0.12.3 run pytest -q              # api-core: 127 tests pass
npm test                                  # web: 8 files / 33 tests pass
uvx uv==0.12.3 run pytest -q              # api-ai: 47 tests pass
uvx uv==0.12.3 run pywrangler dev --help  # pass for api-core/api-ai
wrangler deploy (staging)                 # seven service Workers uploaded
...
voice message transcription POST       # Web multipart + desktop linear16 PCM → Workers AI Whisper; both live staging paths HTTP 200 with identical transcript
...
public app catalog projection           # approved/non-disabled/non-persona app rows and authenticated single-app detail/install state in D1; private fields rejected at backfill; dynamic writes remain legacy → unit + edge verified
...
staging deploy health gate           # one command publishes seven service Workers, then checks Edge/Auth-ready/Core/AI/Realtime/Jobs/rate-limit → verified
authenticated staging smoke          # required release credential plus Web→Edge conversation/enabled-app/memory/chat reads and voice input boundary → live staging verified
...
随后补齐公开 app catalog projection：`cf_app_catalog` 只保存经过回填校验的
公开字段，`GET /v1/approved-apps` 与鉴权的 `GET /v1/apps/popular` 从 D1 读取
approved、非 disabled、非 persona 记录；staging fixture 实测返回 HTTP 200，未
鉴权 popular 返回 401。回填器会拒绝 reviews、支付标识、凭据和 prompt 等私有
字段；随后将 `GET /v1/apps/enabled`、`POST /v1/apps/enable`、`POST
/v1/apps/disable` 的免费公开 app 安装关系投影到 `cf_user_enabled_apps`，并在
D1 中维护幂等 install counter。付费、private/persona、外部 setup callback、app
创建、审核、订阅、MCP 和用户 review 仍由 legacy authority 负责。staging 已实测
`enable 200 → 重复 enable 200 → list 200 → disable 200 → 重复 disable 404`，并
验证 counter 回到 0，临时 fixture 已清理。

随后将 `/v2/apps` 的公开 capability/category/grouped marketplace read 和鉴权的
`/v2/apps/search` 接到同一份 D1 catalog projection，保留分页和 score ordering；
公开 route 不伪造用户 `enabled` 状态，search 支持 q/category/capability/rating/
sort 以及 uid-scoped `installed_apps` 过滤。staging 已验证 grouped、capability、
category、capability-grouped、search 以及 enable → installed search → disable
链路均为 HTTP 200，搜索结果正确标记 `enabled: true`，临时 catalog rows 已清理。
...
2026-08-30 将 17 条非 OAuth 的 `/v1/mcp/*` 数据 API 及 memory、conversation、
action-item 三条语义搜索路由迁入 API Core，
MCP key 继续使用仅保存 SHA-256 digest 的 D1 authority，并由 Edge 按 key subject
执行 memory/action-item 写限流。语义搜索使用四个隔离 Vectorize index；英文专用
BGE-base 资格测试暴露中文任务排序错误后，改用 Workers AI 多语言 BGE-M3 1024 维重嵌入。
D1 outbox + Queue/Jobs cron 维护 projection；Vectorize 只返回
候选 ID，所有结果必须回到 uid-scoped D1 authority hydrate。全量发布验证通过 296 个 TypeScript、179 个 API
...
以及 memory/action item 的创建、编辑、完成和删除；额外使用字面不同的查询实测三类
semantic recall。BGE-M3 中文资格样本中目标 memory/action-item 均排第一，memory scores
为 `0.785/0.6628/0.551`；英文 conversation transcript/summary 合并和 UTC 日期 metadata
filter 在 metadata WAL 收敛后均通过。会话 D1 写入与 vector outbox 同批提交并发送 Queue
hint，live 路径在 19.9 秒后可搜索，不再依赖五分钟 repair cron。撤销后再次访问返回 HTTP
403，临时 key、权威测试数据、projection state 和 outbox 均已
回读确认为清理完成。右侧已登录浏览器刷新
...
`e38069ec-18c1-4e60-bffb-6d349895f09c`、Rate Limit
`6b9b7bf3-0ea0-4e7f-b8f5-ee0e76deb589`、API Core
`f610b763-cc0d-407b-b8ff-098bc5180f67`、API AI
`17752ca1-5b4d-4e09-96cd-d3c5059efad4`、Realtime
