...
- `manifests/routes.yaml` 与 `manifests/resources.yaml`：134 条首期路由和 10 个 staging 资源；`npm test` 前置校验会检查字段、命名空间、重复项、禁止 broad `/v1/*` ownership 及 Edge 路由表示。Edge 只把显式迁移的 route 送入 partial Worker，未迁移的认证 route 在配置 `LEGACY_BACKEND_URL` 时回旧后端。
...
conversation D1 projection         # pre-transcribed uid/id upsert + bounded list/count/detail with locked-row redaction; finalization/memory/search remain legacy → staging candidate
