...
| Web 实时 | `POST /v1/realtime/web-ticket` → 30 秒一次性票据 → `wss …/v4/web/listen` 首帧 `{type:"auth", ticket, device_id_hash}` | 需新增后端路由（fork 路径 `backend/routers/fork/realtime_ticket.py`，可回推上游） | ✅ |
...
| 账户激活围栏 | `409 account data plane not active` **仅在托管云→新后端迁移时启用**；`account_activation_fence=false` 为新品牌默认 | 无 | 改为读 profile 开关 |
...
| `sync_upload_batch_limit` | 5 | 5 | **2** | `local_wal_sync.dart:31` 的 CF 100MB 限制改为读 profile |
...
| shim：后端 `utils/identity.py`、`auth_shim.py`、`config/push_provider.py`、`storage.py`/`storage_minio.py`、`cloud_tasks_redis.py`、`egress_policy.py` | 保留；`OMI_DEPLOYMENT_PROFILE` 值统一为 `omi_cloud/self_hosted/cloudflare`（`neutral` 作别名一版后删除） |
...
| CF：Web vinext 构建、同源代理、web-ticket、`AuthProvider.tsx` 双模式 | 保留；`NEXT_PUBLIC_AUTH_MODE` 改为读 profile；代理模块抽成运行时无关 |
| CF：`rate_limit_config.py` +12 策略、`export_openapi.py` 的 `cloudflare-route-inventory` 面、`test_openapi_contract.py` 三个用例、`checks-manifest.yaml` 三条检查 | 策略表合入（通用）；route-inventory 面合入（对上游无害）；三条检查移入 `checks-manifest.fork.yaml` |
...
- 身份契约套件 `contracts/auth/`：对任一目标的 Auth 服务跑同一组 HTTP 用例（sign-up → sign-in → `set-auth-token` → `/token` → JWKS 校验 → 过期刷新 → sign-out → Web cookie 流 → web-ticket）；`fork-contract-selfhost.yml` 与 `fork-contract-cloudflare.yml` 都跑。
