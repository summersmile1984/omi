...
| D2 | 身份契约 v1：路径 `/api/auth`；JWKS 本地校验；JWT TTL 可配（默认 3600s）；移动端真实 Better Auth 流程；Web cookie + 同源代理 + web-ticket | 采纳 | S2, S4, S5, S6, M2 |
| D3 | Web 运行时：上游 Next.js 源唯一；自托管 standalone 容器；Cloudflare vinext；Moonshine 归档不合入 | 采纳 | S4, M1 |
| D4 | Cloudflare 未迁移路由：支持 `ORIGIN_BACKEND_URL` 指向自托管后端（混合）；未配置则 404 + 能力表 | 采纳（混合为可选） | M2 |
...
| D10 | 上游文件"禁改清单"与 ≤5 行注入点规则成为 fork 纪律并由 `fork-upstream-touch` 守卫 | 采纳 | C1 |
...
| S2 | app: 统一 deployment profile + Better Auth 客户端 | S1 | 5 | `env_test.dart` 改表后绿；三种 `OMI_APP_PROFILE` 构建登录闭环 |
| S3 | desktop: Windows/macOS profile 增加 cloudflare + 统一配置模块 + 日历 Worker 门控 | S1 | 4 | 两端 profile 测试绿；命名 bundle 连 `wrangler dev` 登录 |
| S4 | web: profile 对象 + 同源代理 + web-ticket + vinext 加法（Next 源不变） | S1 | 4 | `web-checks` 绿；`build:vinext:staging` 绿；Playwright 登录用例双运行时通过 |
| S5 | backend: identity/push/storage/queue/egress 注入点最小化 + provider 注册表 + rate-limit 策略 + route-inventory 面 + web-ticket 路由 | S1 | 5 | `backend/test.sh` 绿；`fork-upstream-touch` ≤5 行；两种 `AUTH_PROVIDER` 启动校验 |
...
| B3 | app: 190 ARB 键占位化 + 556 处硬编码 + 字体替换 | B2 | 6 |
| B4 | backend: brand.py + 30 处 prompt + 通知/模板/OpenAPI/前缀 + share_links + firmware 映射 + 制品名 | B0, S5 | 5 |
...
|---|---|---|---|
| 1 | S0 协作解冲突；S1 | S0；C0、C1 | D1–D10 签字；品牌名商标检索；开发者账号申请 |
...
- 上游文件改动：<n> 个，每个 ≤5 行（fork-upstream-touch 输出粘贴）
...
| Moonshine 归档引发自托管 Web 资源顾虑 | 4C8G 上 Next standalone 内存偏高 | `bun` 运行 standalone；或 `output: 'export'` 静态化非 SSR 页面 |
