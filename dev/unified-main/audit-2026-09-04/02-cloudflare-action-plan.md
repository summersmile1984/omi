...
本文是三路行动方案的 Cloudflare 分册；标准服务器见 `01-server-action-plan.md`，终端白牌见 `03-whitelabel-action-plan.md`。以同一条 `main`、短期 PR 分支、`deploy/<target>` 与 `brand/<id>` 两个配置维度推进，不恢复部署专用长期分支。
...
- 本轮没有改产品源码、已跟踪锁文件、部署配置或远端资源；没有部署、创建生产测试账户、访问客户数据。远端操作仅迁移状态查询及公开 readiness。依赖安装和本地 D1 数据在独立 worktree/临时目录；本地 Auth 进程已停止。
...
- **依赖**：`AUTH-1`、`CLIENT-1`；可先建立协议夹具，不先另发“改header常量”PR。
...
- **依赖**：可行性检查可与CF-1/AUTH-1并行；最终交付依赖AUTH-1/CLIENT-1及品牌域。若adapter仍未实现，保持纯CF Web项未完成，不能以外置Bun服务或只发布API替代用户目标。
...
- **核心测试**：`omi-upstream`零diff；两个测试品牌输出互不相交；同品牌不同stage隔离；缺域/ID/secret映射失败；绑定依赖闭包、迁移目录和回滚引用均来自同一渲染结果；重复渲染幂等。
...
第一批并行做 **CF-1、AUTH-1、WEB-1可行性/构建契约**，白牌路同步推进CLIENT-1。第二批做CF-2、CF-3、CF-4和CI-1的target runner，使用同一身份、profile和协议夹具。第三批在CI-1闭环后执行CF-5的当前main staging发布和真实终端验收。

每个包只在其完整可验收表面通过后合入，提交附本轮失败实例、核心/错误测试、`scripts/pr-preflight --suggest`与failure-class判定；按普通PR合入唯一main。整路线的完成条件是当前main对同一白牌客户端/profile可复现完整Cloudflare工件和真实主流程，且证据绑定该SHA；“测试绿”“旧线上健康”“某个Worker能打包”都只是其中一层。
