...
| 契约同一 | 服务 `docs/api-reference/*openapi*.json` 全集 | 服务 `manifests/routes.yaml` 中 `target_runtime ≠ legacy|blocked` 的子集，其余按 §4 处理 | `export_openapi.py --surface cloudflare-route-inventory --check`（CF 已有）+ `contracts/` 套件对两者跑 |
...
| 项 | 改动 |
|---|---|
| 镜像 | `BACKEND_IMAGE` 由 `deploy/self-host/Dockerfile` 构建：`FROM` 上游 `backend/Dockerfile` 产物层 + `pip install -r backend/requirements-fork.txt`（不改上游 requirements/锁文件） |
...
| 实时票据 | `backend/routers/fork/realtime_ticket.py` 提供 `POST /v1/realtime/web-ticket`（Redis 30 秒一次性票据）；`/v4/web/listen` 首帧接受 `ticket`（与 CF 同协议） |
...
| 未迁移路由 | `LEGACY_BACKEND_URL` 语义改为 **`ORIGIN_BACKEND_URL`**：可指向同一品牌的自托管后端（混合部署，§4）；未配置时保持 `404 route not migrated` 并在 `/v1/capabilities` 暴露未提供的路由族，客户端按 profile 隐藏入口 |
| 账户围栏 | `cloudflareProductTrafficDenial`（409）读 profile `account_activation_fence`，新品牌默认关闭 |
| 请求限制 | `sync_upload_batch_limit=2`、`max_request_bytes=100MB` 进 profile（客户端读表） |
| Web | vinext 构建保留（`web/app/vite.config.ts`、`wrangler.jsonc`、`next.config.js` 的 `VINEXT_BUILD` 别名为**加法**，不替换上游 Next 配置）；`NEXT_PUBLIC_AUTH_MODE` 退役为 profile |
...
| 实时票据 | 协议在 `contracts/realtime/web-ticket.md`；Python 实现 `backend/routers/fork/realtime_ticket.py`；CF 实现 `workers/edge`（已有） | 两端同一首帧格式 |
| 限流策略表 | `backend/utils/rate_limit_config.py`（CF 分支 +12 条通用策略） | 自托管 Redis 与 CF DO 都读这张表（CF 已镜像） |
| 路由清单 | `deploy/cloudflare/manifests/backend-routes.json` 由 `export_openapi.py --surface cloudflare-route-inventory` 生成 | 新上游路由出现即失败，强制分类（CF 分支 2026-08-29 404 事故的守卫） |
...
- **推荐**：保留上游 Next.js 源为唯一 Web 源码；自托管目标用 `next build` standalone 容器（上游 `web/app/Dockerfile` 已有）；Cloudflare 目标用 vinext；Moonshine 迁移打 tag `archive/web-moonshine-2026-09` 后不合入。若 Bun/Moonshine 的动机是自托管资源占用，改为在 `deploy/self-host` 用 `bun` 运行 Next standalone（`bun .next/standalone/server.js`）即可，不需要换框架。
