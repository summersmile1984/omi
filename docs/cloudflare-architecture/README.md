...
| CF-03 Better Auth          | 部分完成                | 独立 D1、signup/login/session/JWT、staging 浏览器登录；内部身份查询/原子删除/residual；Jobs 已有覆盖全部身份列与 7 个用户 R2 前缀的只读 product residual；Firebase password verifier、首次登录原生 rehash、参数化 D1 import ledger/checksum/replay tooling；真实 workerd+D1 已验证错误密码不写、成功升级和移除迁移密钥后再登录 | 真实 OAuth/link、破坏性产品数据删除 Workflow 与 mutation fence、真实 Firebase export 导入演练、JWKS retired/grace live 证据、abort/retry、生产身份连续性 |
...
| CF-06 Queues/Workflows     | 部分完成                | transcription Queue、幂等/重复/冲突/终态、DLQ 相关验证；账号删除的 signed D1/R2 residual inventory 与 schema 覆盖 guard                                                                                                                                                                   | 账号删除的破坏性 Workflow/mutation fence/provider cleanup、其他高风险 finalization contracts、积压恢复与 replay 证据         |
...
- Cloudflare TypeScript：27 个文件、199 个测试通过。
