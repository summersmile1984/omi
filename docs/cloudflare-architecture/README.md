...
| 项目                       | 状态                    | 已有证据                                                                                                                                                                                                                                                                                 | 尚未闭合                                                                                                                    |
| -------------------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| CF-00 清单/scaffold        | 已完成                  | hermetic FastAPI 注册表确认 577 条唯一后端路由（573 HTTP、4 WebSocket，含显式 HEAD/OPTIONS）；439 条已匹配 Cloudflare staging owner，138 条显式保留 legacy owner；457 条 Cloudflare 路由、22 个隔离资源、34 个 Redis family、7 个 vector namespace、9 个 R2 namespace 均有机械校验       | 未迁路由的实现工作归入 CF-10，不再是清单未知项                                                                              |
| CF-01 Edge                 | staging 已验证          | header 剥离、防重放、CORS、owner 标记、health/smoke                                                                                                                                                                                                                                      | 生产回滚与非幂等超时的 live 证据                                                                                            |
| CF-02 Python Workers       | staging 已验证          | api-core/api-ai、246+81 个 Python 测试、真实 Worker smoke；API AI 已验证跨 Worker DO binding                                                                                                                                                                                             | 生产规模 bundle/CPU/内存/cold-start/外连预算                                                                                |
...
- Cloudflare TypeScript：47 个测试文件、367 个测试通过；类型检查和生产依赖审计通过。
- `api-core`：246 个测试通过；`api-ai`：81 个测试通过。
- Web：8 个测试文件、38 个测试通过；vinext staging build 与 Worker dry-run 通过。
- manifest：457 条 Cloudflare 路由、577 条完整 backend 路由和 22 个 staging 资源通过校验；138 条 legacy-owned 路由成为可量化迁移队列。
