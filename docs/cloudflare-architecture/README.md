...
| 项目                       | 状态                    | 已有证据                                                                                                                                                                                                                                                                                 | 尚未闭合                                                                                                                    |
| -------------------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| CF-00 清单/scaffold        | 已完成                  | hermetic FastAPI 注册表确认 577 条唯一后端路由（573 HTTP、4 WebSocket，含显式 HEAD/OPTIONS）；311 条已匹配 Cloudflare staging owner，266 条显式保留 legacy owner；323 条 Cloudflare 路由、20 个隔离资源、34 个 Redis family、7 个 vector namespace、9 个 R2 namespace 均有机械校验       | 未迁路由的实现工作归入 CF-10，不再是清单未知项                                                                              |
...
- Cloudflare TypeScript：39 个测试文件、311 个测试通过；类型检查和生产依赖审计通过。
- `api-core`：184 个测试通过；`api-ai`：61 个测试通过。
- Web：8 个测试文件、37 个测试通过；vinext staging build 与 Worker dry-run 通过。
- manifest：323 条 Cloudflare 路由、577 条完整 backend 路由和 20 个 staging 资源通过校验；266 条 legacy-owned 路由成为可量化迁移队列。
