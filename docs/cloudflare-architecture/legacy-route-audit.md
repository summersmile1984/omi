...
截至 2026-08-31，`backend-routes.json` 中还有 79 条 `legacy-owned` 路由。这个清单不是把路由简单改成 Cloudflare 代理：只有当数据 authority、认证边界、异步重试和外部 provider 语义都能在 Workers 上闭合时，才允许把 owner 改成 `staging-owned`。
...
| Desktop release mutation/manifest | beta admission/promotion、breakglass | beta admission/promotion CAS、release pipeline 回填和生产 Firestore→D1 回放 | immutable manifest、macOS Stable channel pointer、legacy release bridge（`/updates/releases` POST/PATCH）和 cache-invalidation contract 已进入 D1；manifest GET/POST、Stable promotion、release bridge、`clear-cache` 在 staging 由 API Core 提供，下一步回填发布流水线并复验 Beta 晋级族 |
