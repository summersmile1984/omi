...
截至 2026-08-31，`backend-routes.json` 中还有 85 条 `legacy-owned` 路由。这个清单不是把路由简单改成 Cloudflare 代理：只有当数据 authority、认证边界、异步重试和外部 provider 语义都能在 Workers 上闭合时，才允许把 owner 改成 `staging-owned`。
...
| Desktop release mutation/manifest | `/v2/desktop/releases*`、beta admission/promotion | immutable manifest 全字段、发布/晋级 compare-and-swap、release pipeline | 先扩充 D1 manifest projection 并做回放，再迁移 GET manifest 和写入/晋级族 |
...
1. 补齐 desktop immutable manifest 的 D1 字段和 release-pipeline 回填，再评估 `GET /v2/desktop/releases/{release_id}`。
