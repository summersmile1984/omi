...
截至 2026-08-31，`backend-routes.json` 中还有 71 条 `legacy-owned` 路由。这个清单不是把路由简单改成 Cloudflare 代理：只有当数据 authority、认证边界、异步重试和外部 provider 语义都能在 Workers 上闭合时，才允许把 owner 改成 `staging-owned`。
...
2. 建立 conversation finalization 的 Jobs/D1 lease projection，成组迁移 status、finalize、reprocess 和 merge。
