...
<AccordionGroup>
  ...
  <Accordion title="Integration Tools" icon="plug">
    ...
    | `web_search_tool` | Deployment-selected search transport | Current public web search |
  </Accordion>
...
|-------|----------|----------|
| `gpt-5.4-mini` / `gpt-5.4` | Core chat, conversation processing | Premium / Max QoS profiles |
| `gpt-4.1-mini` | Fast classification, date extraction | `requires_context()`, filters |
| `claude-sonnet-4-6` | Chat agent (all QoS tiers) | `chat_agent` profile |
| `gemini-2.5-flash-lite` | Session titles, followups | Light tasks (premium) |
