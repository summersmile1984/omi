...
- [ ] Migrations, backup/restore, secrets, TLS/public origins, readiness,
      ...
      now use the repository's authenticated `omi-backup-aead-v1` envelope with
      an explicit operator-provided key file; key custody, production KMS
      integration, and a real encrypted restore drill remain operator
