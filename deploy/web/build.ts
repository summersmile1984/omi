...
export async function buildWeb(options: {
  ...
  if (
    ...
    throw new Error(
      "Select self_hosted or cloudflare and a local/beta/production stage"
    );
  }
  ...
  if (options.manifest)
    profileCommand.push("--manifest", resolve(options.manifest));
  ...
  const presentation = await applyPresentation(source, input);
  ...
  run(
    [resolve(webRoot, "node_modules/.bin/tsc"), "--project", stagedTypes],
    source
  );
  ...
  run(
    [resolve(webRoot, "node_modules/.bin/moonshine"), "build"],
    source,
    environment
  );
  ...
  if (options.target === "cloudflare") {
    await compileWorker(
      generated,
      environment,
      `${input.brand_id}-web-${options.stage}`
    );
  } else {
  ...
  if (options.target === "cloudflare") {
    await cp(
      resolve(generated, "cloudflare"),
      resolve(artifact, "cloudflare"),
      {
        recursive: true,
      }
    );
    await cp(
      resolve(generated, "wrangler.json"),
      resolve(artifact, "wrangler.json")
    );
  } else {
...
if (import.meta.main) {
  ...
  if (!values.target || !values.stage || !values.output)
    throw new Error(
      "Required: --target --stage --output [--brand ID | --manifest PATH]"
    );
  await buildWeb({
