...
`verify-recording.py` is the reusable product acceptance path. It accepts a valid
synthetic account JWT and real 16 kHz mono PCM WAV, exercises `/v4/listen`, checks
durable recognized segments, uses the public speaker-assignment and explicit
finalize routes, waits for the real queue finalizer, then requires canonical
memory and retrieves it through `/v2/messages`. It never writes a transcript or
memory directly. A generated spoken fixture proves TTS-to-ASR software flow, not
a microphone or hardware. CI may control only inference providers while keeping
these public handlers, queue and stores; that is business-contract coverage, not
model qualification.
