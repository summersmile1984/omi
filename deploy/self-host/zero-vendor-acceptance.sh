...
OPERATOR_EVIDENCE_TOOL="$ROOT/deploy/self-host/operator_evidence.py"
...
self_check() {
  local path
  for path in "$CHECKER" "$OPS" "$COMPOSE_WRAPPER" "$E2E_RUNNER" "$E2E_GUARD" "$AUTH_SMOKE" "$LIVE_REPLACEMENT_SMOKE" "$CUTOVER_GATE" "$CUTOVER_SMOKE" "$CUTOVER_OVERLAY" "$CUTOVER_PROXY" "$REALTIME_RELAY_FIXTURE" "$EGRESS_POLICY_CONTRACT" "$EVIDENCE_BUILDER" "$OPERATOR_EVIDENCE_TOOL" "$RUNTIME_EVIDENCE_TOOL" "$RUNTIME_PROVIDER_SCHEMA" "$PUBLIC_OBJECT_EVIDENCE_TOOL" "$SOURCE_WRITE_FREEZE_TOOL" "$AGENT_VM_RECONCILE_TOOL" "$AGENT_VM_RECONCILE_TEST"; do
    [[ -f "$path" ]] || { echo "error: acceptance dependency missing: $path" >&2; return 1; }
  done
  "$PY" -m py_compile "$AUTH_SMOKE" "$LIVE_REPLACEMENT_SMOKE" "$CUTOVER_SMOKE" "$REALTIME_RELAY_FIXTURE" "$EGRESS_POLICY_CONTRACT" "$EVIDENCE_BUILDER" "$OPERATOR_EVIDENCE_TOOL" "$RUNTIME_EVIDENCE_TOOL" "$RUNTIME_PROVIDER_SCHEMA" "$PUBLIC_OBJECT_EVIDENCE_TOOL" "$SOURCE_WRITE_FREEZE_TOOL" "$AGENT_VM_RECONCILE_TOOL"
...
if [[ "$MODE" != contracts ]]; then
  ...
  if [[ "$MODE" == external-cutover-live ]]; then
    ...
    : "${SELF_HOST_MODEL_PROVENANCE_EVIDENCE:?external cutover requires SELF_HOST_MODEL_PROVENANCE_EVIDENCE pointing to operator MOSS provenance evidence}"
    if [[ "$SELF_HOST_MODEL_PROVENANCE_EVIDENCE" != /* || ! -f "$SELF_HOST_MODEL_PROVENANCE_EVIDENCE" || -L "$SELF_HOST_MODEL_PROVENANCE_EVIDENCE" ]]; then
      echo "ERROR: SELF_HOST_MODEL_PROVENANCE_EVIDENCE must be an existing non-symlink absolute JSON file" >&2
      exit 1
    fi
