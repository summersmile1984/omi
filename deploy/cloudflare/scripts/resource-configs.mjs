...
function publicConfig(role, config, input, projected, names, origins) {
  ...
  if (["edge", "auth"].includes(role)) {
    config.vars.ALLOWED_ORIGINS = [
      ...new Set([origins.web, origins.share]),
    ].join(",");
...
export function renderResourcePlan({
  ...
  if (sourceOwners.size !== WORKERS.filter((role) => role !== "web").length)
    throw new Error("Worker templates contain duplicate names");
  const screenshotWriter = templates["screen-frame-writer"].config;
  if (
    screenshotWriter.r2_buckets?.length !== 1 ||
    screenshotWriter.r2_buckets[0].binding !== "SCREEN_FRAMES" ||
    screenshotWriter.d1_databases?.length !== 1 ||
    screenshotWriter.d1_databases[0].binding !== "APP_DB" ||
    screenshotWriter.ai ||
    screenshotWriter.services?.length ||
    screenshotWriter.vectorize?.length ||
    screenshotWriter.queues ||
    Object.entries(templates).some(
      ([role, template]) =>
        role !== "screen-frame-writer" &&
        template.config.r2_buckets?.some(
          (item) => item.binding === "SCREEN_FRAMES"
        )
    )
  )
    throw new Error(
      "screenshot bytes require an isolated writer with only APP_DB and SCREEN_FRAMES"
    );
