...
export function renderPhoneHistoryLedgerSql(plan, now = Math.floor(Date.now() / 1_000)) {
  ...
  const statements = plan.entries.filter((entry) => entry.action === "stage").map((entry) => {
    ...
    return (
      ...
      "twilio_sid, friendly_name, verified_at, is_primary, account_generation, plan_hash, action, status, " +
      ...
      `${entry.accountGeneration}, ${sqlLiteral(entry.planHash)}, 'stage', 'planned', NULL, ${entry.createdAt}, ${now} ` +
  ...
  return [
    ...
    "-- It never calls Twilio or inserts cf_phone_numbers; D1 remote file ingestion supplies the transaction.",
