...
async def _enqueue(request: Request, *, uid: str, job_id: str, conversation_id: str, revision: int) -> bool:
        ...
        await queue.send(
            {
                ...
                "payload": {"conversationId": conversation_id, "revision": revision},
            }
        )
    except Exception:
        return False
...
async def process_conversation_finalization(request: Request):
    ...
    job = (
        await env.APP_DB.prepare(
            "SELECT job_id, uid, conversation_id, finalization_revision, status, attempts "
    ...
    if str(conversation.get("status")) == "completed":
    ...
    if str(conversation.get("status")) != "processing":
        return JSONResponse({"error": "conversation is not being finalized"}, status_code=409)
    ...
    language = str(conversation.get("language") or "en")
    ...
    result = {"id": conversation_id, "status": "completed", "discarded": bool(payload.get("discarded"))}
    try:
...
async def get_conversation_finalization_status(request: Request, conversation_id: str):
    ...
    job = (
        await env.APP_DB.prepare(
            "SELECT job_id, status, attempts FROM cf_conversation_finalization_jobs "
