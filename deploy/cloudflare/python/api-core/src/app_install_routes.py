"""D1 projection for the safe subset of app installation state.

The legacy app owner still controls private apps, paid entitlements, external
integration setup callbacks, reviews, and MCP credentials.  This Worker route
only mutates an approved public catalog row when the app is free and has no
setup callback, so an install cannot accidentally skip a provider-side
authorization or subscription check.
"""
...
def _installable(payload: object) -> tuple[bool, int, str | None]:
    ...
    if _flag(payload.get("is_paid")) or payload.get("payment_link") or payload.get("payment_link_id"):
        return False, 403, "paid app installation remains on the legacy owner"
...
async def _load_installable_app(env: object, app_id: str) -> tuple[dict[str, object] | None, JSONResponse | None]:
    try:
...
async def get_enabled_apps(request: Request):
    ...
    try:
        result = await request.scope["env"].APP_DB.prepare(
            "SELECT app_id FROM cf_user_enabled_apps WHERE uid = ? ORDER BY created_at ASC, app_id ASC"
        ).bind(uid).all()
    except Exception:
        return JSONResponse({"error": "enabled apps unavailable"}, status_code=503)
    rows = result.get("results", []) if isinstance(result, dict) else []
    return [str(row["app_id"]) for row in rows if isinstance(row, dict) and isinstance(row.get("app_id"), str)]
...
async def enable_app(request: Request):
    ...
    env = request.scope["env"]
    app, error = await _load_installable_app(env, app_id)
    if error:
        return error
    uid = str(context["uid"])
    try:
        ...
        ).bind(uid, app["id"], int(time.time())).run()
