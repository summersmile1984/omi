...
describe('one brand/stage Cloudflare resource authority', () => {
  it('consumes actual brand/profile rendering for every stage and the documented inventory example', () => {
    const manifest = YAML.parse(
      readFileSync(resolve(root, '../../brand/omi-upstream/manifest.yaml'), 'utf8'),
    );
    ...
    for (const stage of ['local', 'beta', 'production']) {
      ...
      if (stage === 'beta')
        fixture.input = JSON.parse(
          readFileSync(resolve(root, 'fixtures/resource-inventory.example.json'), 'utf8'),
        );
      ...
      for (const role of ['api-core', 'api-ai'])
        expect(JSON.parse(plan.configs[role].config.vars.BRAND_RUNTIME_JSON)).toEqual({
          brand_id: 'cf-alpha',
      ...
      expect(plan.configs.auth.config.vars.ALLOWED_ORIGINS).toBe(
        [fixture.projected.profile.web_base_url, fixture.projected.profile.share_base_url]
  ...
  it('keeps direct Wrangler development identity aligned with its actual manifest', () => {
    ...
    for (const role of ['api-core', 'api-ai'])
      expect(JSON.parse(templates[role].config.vars.BRAND_RUNTIME_JSON)).toEqual({
        brand_id: id,
        display_name,
        ai_persona_name,
      });
  ...
  it('renders real templates across all stages with auth, MCP, CORS and dependency ownership', () => {
    for (const stage of ['local', 'beta', 'production']) {
      const fixture = resourceFixture('alpha', stage),
        plan = render(fixture);
      expect(plan.resources).toHaveLength(28);
      expect(plan.configs.auth.config.vars).toMatchObject({
        ...
        ALLOWED_ORIGINS: [...new Set([plan.origins.web, plan.origins.share])].join(','),
      ...
      if (stage === 'local') {
        ...
        expect(plan.configs.web.config.routes).toEqual(
          expect.arrayContaining([
            { pattern: new URL(plan.origins.web).hostname, custom_domain: true },
            {
      ...
      for (const [role, dependencies] of Object.entries(plan.dependencies))
        for (const dependency of dependencies)
          ...
          ).toBeLessThan(plan.deploy_order.indexOf(plan.configs[role].config.name));
      expect(plan.rollback_order).toEqual([...plan.deploy_order].reverse());
      expect(JSON.stringify(plan)).not.toMatch(/summersmile1984|omi-cf-|omi-web-app/);
  ...
  it('is deterministic and isolates two brands and two stages, including D1 IDs and secret references', () => {
    ...
    expect(() => assertDisjointPlans([alpha, secretCollision])).toThrow('collision');
  });
  ...
  it('refuses unqualified mount paths and ambiguous public owners, including matching Web inputs', () => {
    for (const key of ['api', 'mcp', 'share', 'objects']) {
      const fixture = resourceFixture();
      changedProfile(fixture, `${key}_base_url`, `https://${key}.alpha.invalid/service`);
      expect(() => render(fixture)).toThrow('CF-4');
    }
    const collision = resourceFixture();
    changedProfile(collision, 'web_base_url', collision.projected.profile.auth_base_url);
  ...
  it('preserves explicitly allocated existing resource names and Durable Object migration histories', () => {
    ...
    expect(plan.configs.edge.config.vars.ACCOUNT_ACTIVATION_FENCE_ENABLED).toBe('true');
  ...
  it('fails on unknown bindings, split D1 authority and a deployment dependency cycle', () => {
    ...
    kv.templates.auth.config.kv_namespaces = [{ binding: 'NEW_CACHE', id: 'unowned' }];
  ...
  it('materializes the same plan into Worker, migration and rollback references without rewriting another output owner', () => {
    ...
    for (const [role, entry] of Object.entries(plan.configs)) {
      ...
      if (role.startsWith('api-')) mkdirSync(resolve(directory, 'python_modules'));
    ...
    const config = JSON.parse(readFileSync(resolve(output, 'migrations/app.json')));
    expect(config.d1_databases[0].database_id).toBe(plan.migrations[1].database_id);
    ...
    for (const [relative, broken] of [
      ...
      if (relative === '')
        writeFileSync(resolve(outside, 'resource-plan.json'), JSON.stringify(plan));
      ...
      for (const check of [true, false]) {
        ...
        expect(readdirSync(outside, { recursive: true }).sort()).toEqual(before);
  ...
  it('ties rollback versions to one release attempt and retains D1 rather than inventing a schema reversal', () => {
    ...
    expect(actions.actions.map((action) => action.worker)).toEqual(plan.rollback_order);
    expect(
      actions.actions.every((action) => action.action === 'rollback_worker_version'),
    ).toBe(true);
    const tampered = structuredClone(snapshot);
    tampered.migrations[0].database_id = plan.migrations[1].database_id;
    expect(() => rollbackActions(plan, tampered)).toThrow('migration authority');
    expect(() =>
    ...
    expect(
      rollbackActions(plan, rollbackSnapshot(plan, observed)).actions.at(-1).action,
    ).toBe('no_previous_version');
    observed[plan.deploy_order[0]] = {
      versions: [{ version_id: '11111111-1111-4111-8111-111111111111', percentage: 50 }],
    };
