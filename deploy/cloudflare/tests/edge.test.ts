...
describe("edge gateway", () => {
  ...
  it("routes the namespaced external MCP OAuth seam with the correct auth boundary", async () => {
    ...
    const refresh = await edge.fetch(
      new Request("https://edge.test/v2/cf/apps/mcp/refresh", {
        method: "POST",
        headers: {
          authorization: "Bearer opaque-session",
          cookie: "must-not-forward",
    ...
    const install = await edge.fetch(
      new Request("https://edge.test/v2/cf/apps/mcp/install", {
        method: "POST",
        headers: {
          authorization: "Bearer opaque-session",
          cookie: "must-not-forward",
    ...
    expect(jobsRequests).toHaveLength(5);
    ...
    expect(new URL(jobsRequests[3].url).pathname).toBe("/v2/cf/apps/mcp/refresh");
    expect(jobsRequests[3].headers.get("cookie")).toBeNull();
    expect(jobsRequests[3].headers.get("authorization")).toBeNull();
    expect(jobsRequests[3].headers.get("x-omi-auth-context")).toBeTruthy();
    expect(new URL(jobsRequests[4].url).pathname).toBe("/v2/cf/apps/mcp/install");
    expect(jobsRequests[4].headers.get("cookie")).toBeNull();
    expect(jobsRequests[4].headers.get("authorization")).toBeNull();
  ...
  it("routes exact legacy MCP app paths to Jobs only behind their explicit owner gate", async () => {
    ...
    const start = await edge.fetch(
      new Request("https://edge.test/v1/apps/mcp", {
        method: "POST",
        headers: {
          authorization: "Bearer opaque-session",
          cookie: "must-not-forward",
    ...
    expect(start.status).toBe(200);
    const callback = await edge.fetch(
      new Request("https://edge.test/v1/apps/mcp/callback?code=opaque&state=opaque-state", {
        headers: { cookie: "must-not-forward", authorization: "must-not-forward" },
      }),
      env as never,
    );
    expect(callback.status).toBe(200);
    const refresh = await edge.fetch(
      new Request("https://edge.test/v1/apps/app-1/mcp/refresh", {
        method: "POST",
        headers: {
          authorization: "Bearer opaque-session",
          cookie: "must-not-forward",
    ...
    expect(new URL(jobsRequests[0].url).pathname).toBe("/v1/apps/mcp");
    expect(new URL(jobsRequests[1].url).pathname).toBe("/v1/apps/mcp/callback");
    expect(new URL(jobsRequests[2].url).pathname).toBe("/v1/apps/app-1/mcp/refresh");
    expect(jobsRequests[0].headers.get("x-omi-auth-context")).toBeTruthy();
    expect(jobsRequests[1].headers.get("x-omi-auth-context")).toBeNull();
    expect(jobsRequests[2].headers.get("x-omi-auth-context")).toBeTruthy();
    expect(jobsRequests.every((request) => request.headers.get("cookie") === null)).toBe(true);
  ...
  it("routes explicit archive memory search through the authenticated API Core boundary", async () => {
    ...
    const response = await edge.fetch(
      new Request(
        "https://edge.test/memory/archive/search?include_archive=true&query=coffee",
        {
          headers: {
            authorization: "Bearer opaque-session",
            cookie: "session=must-not-forward",
    ...
    expect(new URL(coreRequests[1].url).pathname).toBe(
      "/memory/archive/search",
    );
    expect(new URL(coreRequests[1].url).search).toContain(
  ...
  it("routes the sync job run boundary to Jobs with only signed Better Auth context", async () => {
    ...
    const env = {
      ...
      AUTH: service((request) => {
        if (request.url.endsWith("/internal/verify")) {
          return Response.json({ uid: "sync-user", authority: "better-auth" });
        }
        return Response.json({ status: "ok" });
      }),
      API_CORE: service(() => Response.json({ status: "ok" })),
    ...
    const body = JSON.stringify({ job_id: "sync-job-1" });
    const response = await edge.fetch(
      new Request("https://edge.test/v2/sync-jobs/run", {
        method: "POST",
        headers: {
          authorization: "Bearer opaque-session",
          "content-type": "application/json",
    ...
    expect(
      decodeAuthContext(forwarded?.headers.get("x-omi-auth-context") ?? null),
    ).toMatchObject({ uid: "sync-user", authority: "better-auth", audience: "jobs" });
    await expect(forwarded?.text()).resolves.toBe(body);
  });
