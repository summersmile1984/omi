...
/// Stable, body-agnostic error classes for operator Better Auth endpoints.
...
/// Native/mobile Better Auth client. It deliberately uses the signed bearer
/// session header returned by the server, never a browser cookie or an
/// anonymous/dev token minting endpoint.
final class BetterAuthClient {
  ...
  Future<BetterAuthSession> signInEmail({required String email, required String password}) {
  ...
  Future<BetterAuthSession> signUpEmail({required String email, required String password, required String name}) {
  ...
  static Map<String, dynamic> _object(String body) {
    try {
      ...
      // Do not expose operator response bodies, which may contain PII.
    }
